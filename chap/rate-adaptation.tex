...
\end{equation}
其中$e(t)$指代过程变量与控制目标在时刻$t$的误差，$K_p$、$K_i$、$K_d$是三个可调参数，依次称为比例增益、积分增益和微分增益。公式(\ref{eq:pid-output})的结果通过某种方式来影响系统状态，使得过程变量朝着控制目标变化，如图\ref{fig:18}\footnote{http://en.wikipedia.org/wiki/PID\_controller}所示。
...
\begin{figure}[!t]
	\centering
	\subfloat[]{\label{fig:19-1}\includegraphics[width=0.6\textwidth]{clip/19-1.png}} \\
	\subfloat[]{\label{fig:19-2}\includegraphics[width=0.6\textwidth]{clip/19-2.png}}
...
\begin{figure}[t]
\centering
\includegraphics[width = 0.6\linewidth]{figures/Fluctuation.png}
